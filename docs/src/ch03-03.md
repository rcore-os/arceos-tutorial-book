...
其实像这种ArceOS在用户看不见的地方根据用户选择的特性需求，组装并扩展各种模块提供用户定制的运行环境以达到目的，其实就是ArceOS的组件化的运作方式的核心机制。我们从最简单的helloworld使用alloc feature，”窥一斑而知全豹“，来初步触及ArceOS的核心运作机制。在后续的章节我们会为读者展示ArceOS更为强大的功能特性，其功能运作的基本原理和helloworld大体相似的，相信读者理解了这部分的内容，后续的章节理解起来会更为顺畅一些。
